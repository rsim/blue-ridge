...
Smoke.Mock.Expectation.stub = function(mock, attr) {
  return function() {
    return function() {
      var matched, return_original, return_value, args = arguments;
      jQuery.each(this, function() {
        if (this.run(args)) {
          matched = true;
          // if and_return was called with function then call this function to get return value
          if (typeof(this.returnValue) === "function") {
            return_value = this.returnValue.apply(mock, args);
            // if value is undefined then look for next expectation
            if (return_value === undefined) {
              return_original = true;
            // if function returned value then stop jQuery.each loop
            } else {
              return false;
            }
          } else {
      ...
      // if all functions returned undefined then run original function
      if (return_value === undefined && return_original) {
        return mock._valuesBeforeMocking[attr].apply(mock, args);
      }
      return return_value;        
